...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_dataset_fpath",
        type=str,
        default="data/holdout_split_batch12_perm/symbolic_batch12_perm_ltl_type_3_42_fold4.pkl",
    ...
    parser.add_argument(
        ...
        default="gpt-4",
        choices=[
            "gpt3_finetuned_symbolic_batch12_perm_utt_0.2_111",
            "gpt-4",
            "text-davinci-003",
        ],
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="number of examples per instance in prompt for GPT",
    )
    parser.add_argument(
        "--rand_eval_samples",
        type=int,
        default=100,
        help="number of random evaluation samples per formula",
    )
    parser.add_argument(
        "--seed_eval_samples",
        type=int,
        default=42,
        help="seed for randomly sampling evaluation samples",
    )
    parser.add_argument(
        "--aggregate",
        action="store_true",
    ...
    parser.add_argument(
        "--aggregate_dpath",
        type=str,
        default="results/pretrained_gpt4/type_holdout_batch12_perm",
        help="dpath to results file to aggregate",
    )
    ...
    if args.aggregate:  # aggregate acc-per-formula result files
        ...
        if (
            "gpt" in args.model or "davinci" in args.model
        ):  # gpt for finetuned gpt3 or off-the-shelf gpt-4, text-davinci-003 for off-the-shelf gpt-3
            ...
            if "finetuned" in args.model:
                ...
                prompt_fpath = os.path.join(
                    "data", "prompt_symbolic_batch12_perm", prompt_fname
                )
                prompt = load_from_file(prompt_fpath)
                valid_iter = [(f"{prompt} {utt}\nLTL:", ltl) for utt, ltl in valid_iter]
                result_dpath = os.path.join(
                    "results", f"pretrained_gpt{gpt_model_number}", dname
                )
                os.makedirs(result_dpath, exist_ok=True)
                result_log_fpath = os.path.join(
                    result_dpath,
                    f"log_{args.model}_{Path(prompt_fname).stem}_{args.rand_eval_samples}-eval-samples.csv",
                )
                acc_fpath = os.path.join(
                    result_dpath,
                    f"acc_{args.model}_{Path(prompt_fname).stem}_{args.rand_eval_samples}-eval-samples.csv",
                )
            ...
            if args.rand_eval_samples:
                ...
                for (pattern_type, nprop), data in meta2data.items():
                    ...
                    valid_meta_sampled.extend(
                        [(pattern_type, props) for _, _, props in examples]
                    )

                dataset["valid_iter"], dataset["valid_meta"] = (
                    valid_iter_sampled,
                    valid_meta_sampled,
                )
                ...
                print(
                    f"test set size: {len(dataset['valid_iter'])}, {len(dataset['valid_meta'])}\n{meta2data.keys()}"
                )
                breakpoint()

            split_dname = os.path.join("data", f"eval_gpt{gpt_model_number}")
            os.makedirs(split_dname, exist_ok=True)
            split_dataset_fpath = os.path.join(
                split_dname, f"{dataset_name}_{args.rand_eval_samples}-eval-samples.pkl"
            )
            save_to_file(dataset, split_dataset_fpath)
            model = (
                GPT4(engine, temp=0, max_tokens=128)
                if gpt_model_number == 4
                else GPT3(engine, temp=0, max_tokens=128)
            )
        else:
