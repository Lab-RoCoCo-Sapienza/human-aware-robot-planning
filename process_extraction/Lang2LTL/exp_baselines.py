...
def run_exp():
    ...
    if args.full_e2e:  # Full end-to-end from language to LTL
        ...
        out_ltls = [
            full_e2e_module.translate(query, full_e2e_prompt) for query in input_utts
        ]

        accs, accumulated_acc = evaluate_grounded_ltl(true_ltls, out_ltls)
        for idx, (input_utt, output_ltl, true_ltl, acc) in enumerate(
        ...
        grounded_utts, objs_per_utt = ground_utterances(
            input_utts, utt2names, name2grounds
        )  # ground names to objects in env
        if args.trans_e2e:
            out_ltls = translate_e2e(grounded_utts)
        else:
            out_ltls, out_sym_ltls, placeholder_maps = translate_modular(
                grounded_utts, objs_per_utt
            )
            ...
            if args.env == "osm":
                ...
                logging.info(
                    f"Language to LTL translation accuracy: {accumulated_acc}\n\n"
                )
...
def rer():
    ...
    for idx_utt, utt in enumerate(input_utts):
        ...
        logging.info(
            f"Extracting referring expressions from utterance: {idx_utt}/{len(input_utts)}"
        )
        names_per_utt = [
            name.strip()
            for name in rer_module.extract_ne(
                query=f"{rer_prompt.strip()} {utt}\nPropositions:"
            )
        ]
...
def ground_names(names):
    ...
    """
    obj2embed = load_from_file(
        obj_embed
    )  # load embeddings of known objects in given environment
...
def translate_modular(grounded_utts, objs_per_utt):
    ...
    if "ft" in translation_engine:
        ...
        raise ValueError(
            f"ERROR: Unrecognized translation engine: {translation_engine}"
        )
    ...
    if "gpt3" in args.sym_trans:
        ...
        (
            ...
            src_vocab_size,
            tar_vocab_size,
        ) = construct_dataset_meta(train_iter)
    ...
    for objs in objs_per_utt:
        placeholder_map, placeholder_map_inv = build_placeholder_map(
            objs, args.convert_rule, PROPS
        )
        placeholder_maps.append(placeholder_map)
        placeholder_maps_inv.append(placeholder_map_inv)
    trans_queries, _ = substitute(
        grounded_utts, placeholder_maps, is_utt=True
    )  # replace names by symbols
    ...
    output_ltls, _ = substitute(
        symbolic_ltls, placeholder_maps_inv, is_utt=False
    )  # replace symbols by props
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="osm",
        choices=["osm", "cleanup", "cleanup_gopalan", "osm_berg"],
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["philadelphia_2", "new_york_1"],
        help="list of cities in OSM or cleanup for cleanup env.",
    )
    parser.add_argument(
        "--holdout",
        type=str,
        default="utt",
        choices=["utt", "formula", "type"],
        help="type of holdout testing.",
    )
    parser.add_argument(
        "--rer",
        type=str,
    ...
    parser.add_argument(
        "--topk", type=int, default=2, help="top k similar known names to re"
    )
    parser.add_argument(
        "--sym_trans",
        type=str,
        default="gpt3_finetuned",
        choices=[
            "gpt3_finetuned",
            "gpt3_pretrained",
            "t5-base",
    ...
    parser.add_argument(
        ...
        help="name to prop conversion rule.",
    )
    parser.add_argument(
        "--full_e2e",
        action="store_true",
        help="solve translation and ground end-to-end using GPT-3",
    )
    parser.add_argument(
        "--full_e2e_prompt",
        type=str,
        default="data/cleanup_full_e2e_prompt_15.txt",
        help="path to full end-to-end prompt",
    )
    parser.add_argument(
        "--nsamples",
        type=int,
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(
                    ...
                    "lang2ltl",
                    args.env,
                    f'log_raw_results_{"_".join(args.cities)}_new.log',
                ),
                mode="w",
            ),
    ...
    if args.env == "osm" or args.env == "cleanup":
        ...
        for city in args.cities:
            ...
            data_fpaths = [
                os.path.join(city_dpath, fname)
                for fname in os.listdir(city_dpath)
                if fname.startswith("symbolic")
            ]
            data_fpaths = sorted(data_fpaths, reverse=True)

            obj_embed = os.path.join(
                env_dpath, "lmk_sem_embeds", f"obj2embed_{city}_{args.embed_engine}.pkl"
            )
            name_embed = os.path.join(
                env_dpath,
                "lmk_name_embeds",
                f"name2embed_{city}_{args.embed_engine}.pkl",
            )
            ...
            for data_fpath in data_fpaths:
                ...
                result_dpath = os.path.join(
                    "results", "lang2ltl", args.env, city, result_subd
                )
                ...
                pair_result_fpath = os.path.join(
                    result_dpath,
                    f"acc_{Path(data_fpath).stem}.csv".replace("symbolic", "grounded"),
                )

                if (
                    os.path.basename(pair_result_fpath) not in os.listdir(result_dpath)
                    and args.holdout in pair_result_fpath
                    ...
                    for (utt, ltl), (
                        ...
                        if "restricted_avoidance" in pattern_type:
                            true_sym_ltls.append(
                                substitute_single_letter(sym_ltl, {props[-1]: PROPS[0]})
                            )
        ...
        obj_embed = os.path.join(
            env_dpath,
            "lmk_sem_embeds",
            f"obj2embed_{args.cities[0]}_{args.embed_engine}.pkl",
        )
        ...
        name_embed = os.path.join(
            env_dpath,
            "lmk_name_embeds",
            f"name2embed_{args.cities[0]}_{args.embed_engine}.pkl",
        )

        result_dpath = os.path.join("results", "lang2ltl", args.env)
        os.makedirs(result_dpath, exist_ok=True)
        all_result_fpath = os.path.join(
            result_dpath, f"acc_grounded_{Path(data_fpath).stem}.json"
        )
        pair_result_fpath = os.path.join(
            result_dpath, f"acc_grounded_{Path(data_fpath).stem}.csv"
        )
        ...
        for (utt, ltl), (pattern_type, nprops, city) in zip(valid_iter, meta_iter):
            city = "_".join(
                [char[1] if "#" in char else char for char in city.lower().split()]
            )
        ...
        for city in cities:
            ...
            obj_embed = os.path.join(
                obj_emebd_dpath, f"obj2embed_{city}_{args.embed_engine}.pkl"
            )
            logging.info(f"known lmk embed: {obj_embed}")
