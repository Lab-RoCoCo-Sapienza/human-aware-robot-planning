...
def finetune_t5(
    model_name, data_fpath, end_idx, model_dpath=None, valid_size=0.2, test_size=0.1
):
    """
    ...
    def compute_metrics(eval_pred):
        ...
        decoded_preds = tokenizer.batch_decode(
            predictions, skip_special_tokens=True, max_length=MAX_TAR_LEN
        )
        decoded_labels = tokenizer.batch_decode(
            labels, skip_special_tokens=True, max_length=MAX_TAR_LEN
        )
        return metric.compute(predictions=decoded_preds, references=decoded_labels)

    in_seqs_train, out_seqs_train, in_seqs_valid, out_seqs_valid = construct_dataset(
        data_fpath, end_idx
    )
    symbolic_dataset = DatasetDict(
        {
            "train": Dataset.from_dict({"utt": in_seqs_train, "ltl": out_seqs_train}),
    ...
    print(
        f"Training set size: {len(in_seqs_train)}, {len(out_seqs_train)}\nValidation set size: {len(symbolic_dataset['validation'])}"
    )
...
def construct_dataset(fpath, end_idx):
    ...
    if end_idx:
        train_iter = train_iter[
            : BASE_DATASET_SIZE + end_idx
        ]  # keep all base dataset, slice composed dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="train and test sets.",
    )
    parser.add_argument(
        "--end_idx", type=int, default=None, help="slicing index for learning curve."
    )
    parser.add_argument(
        "--model_dpath",
        type=str,
    ...
    parser.add_argument(
        ...
        help="huggingface cache.",
    )
    parser.add_argument(
        "--model", type=str, choices=HF_MODELS, help="name of supervised seq2seq model"
    )
