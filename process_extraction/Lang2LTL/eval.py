...
def evaluate_grounded_ltl(true_ltls, out_ltls, string_match=False):
    ...
    for true_ltl, out_ltl in zip(true_ltls, out_ltls):
        if (
            true_ltl == out_ltl
        ):  # TODO: Spot cannot handle long but correct LTL formula, e.g. F & 62_on_the_park U 62_on_the_park & ! 62_on_the_park U ! 62_on_the_park F & 62_on_the_park U 62_on_the_park & ! 62_on_the_park U ! 62_on_the_park F & 62_on_the_park U 62_on_the_park & ! 62_on_the_park U ! 62_on_the_park F 62_on_the_park
            is_correct = "True"
        elif string_match:
            is_correct = "False"
        else:
...
def evaluate_sym_trans(
    ...
    for batch_idx, batch in enumerate(batches):
        ...
        for idx, ((utt, true_ltl), (pattern_type, props, *other_meta)) in enumerate(
            batch
        ):
            ...
            try:  # output LTL formula may have syntax error
                ...
                out_ltl = model.type_constrained_decode([utt])[0].strip()
                try:
                    is_correct = spot.are_equivalent(
                        spot.formula(out_ltl), spot.formula(true_ltl)
                    )
