...
def generate_embeds(
    ...
    if embed_model == "gpt3":
        save_fpath = os.path.join(
            embed_dpath, f"obj2embed_{lmk_fname}_{embed_model}-{embed_engine}.pkl"
        )
    else:
        save_fpath = os.path.join(
            embed_dpath, f"obj2embed_{lmk_fname}_{embed_model}.pkl"
        )
    ...
    for lmk, sem in lmk2sem.items():
        ...
        if lmk not in name2embed or update_embed:
            ...
            print("sem filtered:", sem_filtered)
            name2embed[lmk] = embed_module.get_embedding(json.dumps(sem_filtered))
        """
        name2embed[lmk] = embed_module.get_embedding(sem["description"])
...
if __name__ == "__main__":
    ...
    lmk_fpaths = [
        os.path.join(lmk_dpath, fname)
        for fname in os.listdir(lmk_dpath)
        if "json" in fname
    ]
    ...
    for idx, lmk_fpath in enumerate(lmk_fpaths):
        ...
        _, save_fpath = generate_embeds(
            args.model, env_dpath, lmk_fpath, keep_keys, args.embed_engine
        )
        print(
            f"{idx}: embeddings generated by model: {args.model}-{args.embed_engine}\nstored at: {save_fpath}\n"
        )
