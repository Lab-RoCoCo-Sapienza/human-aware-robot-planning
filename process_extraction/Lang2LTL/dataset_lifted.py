...
from .formula_sampler import FEASIBLE_TYPES, FILTER_TYPES, PROPS, sample_formulas
from .utils import (
    append_ids_to_path,
    deserialize_props_str,
    load_from_file,
    remove_id_from_path,
    save_to_file,
...
def create_symbolic_dataset(
    load_fpath, save_fpath, filter_types, update_dataset, remove_pun, perm_props=False
):
    """
    ...
    if update_dataset or not os.path.isfile(save_fpath):
        ...
        for _, _, pattern_type, nprops, utt in data:
            if remove_pun:
                ...
                )  # remove punctuations for substitution
            pattern_type = "_".join(pattern_type.lower().split())
            if pattern_type not in filter_types:
                ltls, props_perm = sample_formulas(
                    pattern_type, int(nprops), False
                )  # sample ltls w/ all possible perms
                if perm_props:  # all possible permutations of propositions
                    for ltl, prop_perm in zip(ltls, props_perm):
                        sub_map = {
                            prop_old: prop_new
                            for prop_old, prop_new in zip(
                                PROPS[: int(nprops)], prop_perm
                            )
                        }
                        utt_perm = substitute_single_letter(
                            utt, sub_map
                        )  # sub props in utt w/ permutation corres to ltl
                        csv_symbolic.append(
                            [
                                pattern_type,
                                prop_perm,
...
def analyze_symbolic_dataset(data_fpath):
    ...
    for ltl_type, props, utt, ltl in data:
        ...
        props = [
            prop.replace("'", "") for prop in list(props.strip("][").split(", "))
        ]  # "['a', 'b']" -> ['a', 'b']
...
def construct_split_dataset(
    ...
    """
    print(
        f"Generating train, test split for holdout type: {holdout_type}; seed: {seed}"
    )
    dataset = load_from_file(data_fpath)
    dataset_name = (
        f"{'_'.join(Path(data_fpath).stem.split('_')[:2])}_perm"
        if perm_props
        else Path(data_fpath).stem
    )  # remove noperm identifier from dataset name if perm_props=True

    if holdout_type == "ltl_type":  # hold out specified pattern types
        kf = KFold(
            n_splits=len(feasible_types) // size, random_state=seed, shuffle=True
        )
        for fold_idx, (train_indices, holdout_indices) in enumerate(
            kf.split(feasible_types)
        ):
            ...
            train_iter, train_meta, valid_iter, valid_meta = (
                ...
                [],
                [],
            )  # meta data is (pattern_type, nprops) pair
    ...
    ):  # hold out specified (pattern type, nprops) pairs
        all_formulas = []
        for pattern_type, props_str, _, _ in dataset:
            ...
            formula = (
                pattern_type,
                len(props),
            )  # same type and nprops, diff perm of props considered same formula
        ...
        for fold_idx, (train_indices, holdout_indices) in enumerate(
            ...
            train_iter, train_meta, valid_iter, valid_meta = (
                ...
                [],
                [],
            )  # meta data is (pattern_type, nprops) pairs
            formula2count = defaultdict(int)
            for pattern_type, props_str, utt, ltl in dataset:
                props = deserialize_props_str(props_str)
                formula = (pattern_type, len(props))
        ...
        holdout_type == "utt"
    ):  # hold out a specified ratio of utts for every (pattern type, nprops) pair
        train_iter, train_meta, valid_iter, valid_meta = (
            [],
            [],
            [],
        ...
        for (pattern_type, nprops), data in meta2data.items():
            train_dataset, valid_dataset = train_test_split(
                data, test_size=size, random_state=seed
            )
            if firstn:
        ...
        if firstn:
            ...
            split_fpath = (
                f"{split_dpath}/{dataset_name}_{holdout_type}_{size}_{seed}.pkl"
            )
        save_split_dataset(
            split_fpath,
            train_iter,
...
def permute(pattern_type, nprops, utt, data, meta):
    ...
    """
    ltls_perm, props_perm = sample_formulas(
        pattern_type, nprops, False
    )  # sample ltls w/ all possible perms
    for ltl_perm, prop_perm in zip(ltls_perm, props_perm):
        sub_map = {
            prop_old: prop_new for prop_old, prop_new in zip(PROPS[:nprops], prop_perm)
        }
        utt_perm = substitute_single_letter(
            utt, sub_map
        )  # sub props in utt w/ permutation corres to ltl
...
def save_split_dataset_new(
    split_fpath, train_iter, train_meta, valid_iter, valid_meta, info
):
    split_dataset = {
        "train_iter": train_iter,
        "train_meta": train_meta,
...
def generate_prompts_from_split_datasets(split_dpath, prompt_dpath, nexamples, seed):
    ...
    """
    split_fpaths = [
        os.path.join(split_dpath, fname)
        for fname in os.listdir(split_dpath)
        if "pkl" in fname
    ]
...
def generate_prompts_from_split_dataset(split_fpath, prompt_dpath, nexamples, seed):
    ...
    meta2data = defaultdict(list)
    for idx, ((utt, ltl), (pattern_type, props)) in enumerate(
...
def generate_lc_splits(split_fpath, portions=[0.1, 0.3, 0.5, 0.7], seed=42):
    ...
    for portion in portions:
        ...
        split_pkl_new = {
            "train_iter": train_iter_new,
            "train_meta": train_meta_new,
            "valid_iter": valid_iter,
            "valid_meta": valid_meta,
        }
        save_to_file(
            split_pkl_new, os.path.join(os.path.dirname(split_fpath), lc_split_fname)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "data/aggregated_responses_batch1.csv",
            "data/aggregated_responses_batch2.csv",
        ],
        help="fpath to aggregated Google form responses.",
    )
    parser.add_argument(
        "--perm",
        action="store_true",
        help="True if permute props after train, test split.",
    )
    parser.add_argument(
        "--update",
        action="store_true",
        help="True if update existing symbolic dataset w/ new responses.",
    )
    parser.add_argument(
        "--merge",
        action="store_true",
        help="True if merge Google form responses from batches.",
    )
    parser.add_argument(
        "--remove_pun", action="store_true", help="True if remove punctuations."
    )
    parser.add_argument(
        "--seeds_split",
        action="store",
        type=int,
        nargs="+",
        default=[0, 1, 2, 42, 111],
        help="1 or more random seeds for train, test split.",
    )
    parser.add_argument(
        "--firstn",
        type=int,
        default=None,
        help="only use first n training samples per formula.",
    )
    parser.add_argument(
        "--nexamples",
        action="store",
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="random seed for choosing prompt examples.",
    )
    args = parser.parse_args()

    # Merge Google form responses from batches and construct non-permuted symbolic dataset for each batch
    data_fpaths = (
        args.data_fpath if isinstance(args.data_fpath, list) else [args.data_fpath]
    )
    postfix = "".join([f"{i}" for i in range(1, len(data_fpaths) + 1)])
    if args.merge:
        for data_fpath in data_fpaths:
            batch_id = Path(data_fpath).stem.split("_")[-1]
            symbolic_fpath = append_ids_to_path(
                f"data/symbolic_{batch_id}_noperm.csv", args.remove_pun, "nopun", "pun"
            )
            create_symbolic_dataset(
                data_fpath, symbolic_fpath, FILTER_TYPES, args.update, args.remove_pun
            )
            analyze_symbolic_dataset(remove_id_from_path(symbolic_fpath, "pun"))
        data_fpath = merge_batches(data_fpaths)
        symbolic_fpath = append_ids_to_path(
            f"data/symbolic_batch{postfix}_noperm.csv", args.remove_pun, "nopun", "pun"
        )
        split_dpath = append_ids_to_path(
            f"data/holdout_split_batch{postfix}",
            [args.perm, args.remove_pun],
            ["perm", "nopun"],
            ["noperm", "pun"],
        )  # dpath to save train, test split
        prompt_dpath = append_ids_to_path(
            f"data/prompt_symbolic_batch{postfix}",
            [args.perm, args.remove_pun],
            ["perm", "nopun"],
            ["noperm", "pun"],
        )  # dpath to save prompts
    else:
        data_fpath = data_fpaths[0]
        batch_id = Path(data_fpath).stem.split("_")[-1]
        symbolic_fpath = f"data/symbolic_{batch_id}_noperm.csv"
        split_dpath = append_ids_to_path(
            f"data/holdout_split_{batch_id}", args.perm, "perm", "noperm"
        )  # dpath to save train, test split
        prompt_dpath = append_ids_to_path(
            f"data/prompt_symbolic_{batch_id}", args.perm, "perm", "noperm"
        )  # dpath to save prompts
    ...
    seeds = (
        args.seeds_split if isinstance(args.seeds_split, list) else [args.seeds_split]
    )
    for seed in seeds:
        construct_split_dataset(
            symbolic_fpath,
            split_dpath,
            "utt",
            FEASIBLE_TYPES,
    ...
    if args.perm:
        symbolic_fpath = (
            f"data/symbolic_batch{postfix}_perm.csv"
            if args.merge
            else f"data/symbolic_{batch_id}_perm.csv"
        )
        symbolic_fpath = append_ids_to_path(
            symbolic_fpath, args.remove_pun, "nopun", "pun"
        )
        create_symbolic_dataset(
            data_fpath, symbolic_fpath, FILTER_TYPES, args.update, args.remove_pun, True
        )
        analyze_symbolic_dataset(remove_id_from_path(symbolic_fpath, "pun"))
    construct_split_dataset(
        symbolic_fpath,
    ...
    generate_prompts_from_split_datasets(
        split_dpath, prompt_dpath, args.nexamples, args.seed_prompt
    )
