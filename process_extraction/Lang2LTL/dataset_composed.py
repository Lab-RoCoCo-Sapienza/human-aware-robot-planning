...
def construct_composed_dataset(data_fpath, composed_fpath):
    ...
    dataset["train_iter"].extend(
        dataset["valid_iter"]
    )  # all base dataset used for train
    ...
    if "zeroshot" in composed_fpath:
        ...
        for train_data, train_meta, valid_data, valid_meta, info in composed:
            ...
            save_split_dataset_new(
                split_fpath, train_data, train_meta, valid_data, valid_meta, info
            )
        ...
        raise ValueError(
            f"ERROR: unrecognized holdout type in compose_fpath: {composed_fpath}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="original symbolic dataset.",
    )
    parser.add_argument(
        "--composed_fpath",
        type=str,
        default="data/composed_utt_symbolic_batch12_noperm.pkl",
        help="composed dataset.",
    )
