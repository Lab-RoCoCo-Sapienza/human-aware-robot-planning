...
MODEL_NAMES = {
    "finetuned_gpt3": "Finetuned GPT3",
    "pretrained_gpt3": "Prompt GPT3",
    "s2s_pt_transformer": "Seq2Seq",
}
TEST_NAMES = {
    "utt_holdout": "Utterance",
    "formula_holdout": "Formula",
    "type_holdout": "Type",
}
...
def get_full_system_piechart(test_type):
    ...
    for city in cities:
        ...
        if os.path.exists(filepath1):
            files = [
                file for file in os.listdir(filepath1) if fnmatch(file, "acc*.csv")
            ]
            for file in files:
                df = pd.read_csv(os.path.join(filepath1, file))
                if "False" in df["Accuracy"].value_counts().index:
                    symbolic_errors = (
                        symbolic_errors + df["Accuracy"].value_counts()["False"]
                    )
                if "RER or Grounding Error" in df["Accuracy"].value_counts().index:
                    RER_errors = (
                        RER_errors
                        + df["Accuracy"].value_counts()["RER or Grounding Error"]
                    )
...
def parse_per_city_accs(test_type):
    ...
    for city in cities:
        ...
        if os.path.exists(filepath1):
            files = [
                file for file in os.listdir(filepath1) if fnmatch(file, "acc*.json")
            ]
            for file in files:
...
def create_symbolic_accuracies_table(
    test_types=SYMBOLIC_TEST_TYPES, model_types=SYMBOLIC_MODEL_TYPES
):
    entries = []
...
def create_osm_accuracies_table(
    test_types=SYMBOLIC_TEST_TYPES, model_types=OSM_MODEL_NAMES
):
    entries = []
    for test_type in test_types:
        for model_type in model_types:
...
def get_copynet_accuracies(test_type):
    ...
    relevant_dirs = [
        os.path.join(filepath, dirname)
        for dirname in os.listdir(filepath)
        if test_string in dirname and os.path.isdir(os.path.join(filepath, dirname))
    ]
    accs = []
...
def get_n_prop_accuracies(csv_data, model_id, test_type):
    ...
    for formula_type in formula_types:
        ...
        if len(data) > 0:
            acc = np.sum(
                data["Number of Utterances"] * data["Accuracy"].astype(float)
            ) / np.sum(data["Number of Utterances"])
            entry = {}
            entry["Model ID"] = model_id
...
def resolve_filenames(test_type, model_type):
    ...
    filenames = [
        ...
        and "acc" in file
        and "num_prop" not in file
        and "formula_type" not in file
    ]
    # print(filenames)
...
def parse_acc(csv_data):
    csv_data = csv_data.loc[csv_data["Accuracy"] != "no valid data"]
    num = pd.np.sum(
        csv_data["Number of Utterances"] * csv_data["Accuracy"].astype(float)
    )
