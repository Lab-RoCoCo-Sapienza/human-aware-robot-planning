...
def rename_map_files(osm_lmks_dpath):
    ...
    mapid2envname = {
        ...
        "Map05": "Chicago #2",
        "Map07": "Houston",
        "Map10": "Philadelphia #1",
        "Map11": "Philadelphia #2",
        "Map12": "San Diego #1",
        "Map13": "San Diego #2",
        "Map14": "Jacksonville #1",
        "Map15": "Jacksonville #2",
        "Map16": "Columbus #1",
        "Map17": "Columbus #2",
        "Map18": "Charlotte #1",
        "Map19": "Charlotte #2",
...
def construct_grounded_dataset(
    split_dpath, lmks, env, remove_perm, seed, nsamples, add_comma, model, save_dpath
):
    ...
    split_fnames = sorted(
        [fname for fname in os.listdir(split_dpath) if "pkl" in fname]
    )
    ...
    for split_fname in split_fnames:
        ...
        train_iter, train_meta, valid_iter, valid_meta = (
            dataset["train_iter"],
            dataset["train_meta"],
            dataset["valid_iter"],
            dataset["valid_meta"],
        ...
        logging.info(
            f"old train, test size: {len(train_iter)} {len(train_meta)} {len(valid_iter)} {len(valid_meta)}"
        )
        logging.info(
            f"test size before remove prop perm: {len(valid_iter)} {len(valid_meta)}"
        )
        if remove_perm:  # remove prop perms from valid_iter
            valid_iter, valid_meta = remove_prop_perms(valid_iter, valid_meta, PROPS)
        unique_formulas = set(
            [(pattern_type, len(props)) for pattern_type, props in valid_meta]
        )
        ...
        logging.info(
            f"test size after remove prop perm: {len(valid_iter)} {len(valid_meta)}\n"
        )

        logging.info("generating grounded train")
        dataset["train_iter"], dataset["train_meta"] = substitute_lmks(
            train_iter, train_meta, lmks, seed, add_comma, model
        )
        logging.info("generating grounded valid")
        start_time = time.time()
        dataset["valid_iter"], dataset["valid_meta"] = substitute_lmks(
            valid_iter,
            valid_meta,
            lmks,
            seed + 10000000,
            ...
            nprops2lmkperms,
            nsamples,
        )  # +10000000 avoid sampele lmks w/ same seeds as train set
        ...
        logging.info(
            f"new train, test size: {len(dataset['train_iter'])} {len(dataset['train_meta'])} {len(dataset['valid_iter'])} {len(dataset['valid_meta'])}\n\n"
        )


def substitute_lmks(
    data, meta_data, lmks, seed, add_comma, model, nprops2lmkperms=None, nsamples=None
):
    data_grounded, meta_data_grounded = [], []

    for idx, ((utt, ltl), (pattern_type, props)) in enumerate(zip(data, meta_data)):
        props = [
            prop.translate(str.maketrans("", "", string.punctuation)).strip()
            for prop in props
        ]  # ['a,'] -> ['a']
        props_norepeat = list(
            set(props)
        )  # repeat props in restricted avoidance, e.g., [a, a, a]
        seed += idx  # diff seed to sample diff lmks for each utt-ltl pair
        if nsamples:  # permute lmks for valid_iter
            nprops_norepeat = len(
                props_norepeat
            )  # restricted avoidance repeat props, e.g., [a, a, a]
            if nprops_norepeat not in nprops2lmkperms:
                nprops2lmkperms[nprops_norepeat] = list(
                    permutations(lmks, nprops_norepeat)
                )  # each perm contain nprops_norepeat lmks
        ...
        for lmk_sub in lmk_subs:  # substitute multiple lmks in each (utt, ltl)
            utt_grounded, ltl_grounded, lmk_names = substitute_lmk(
                utt, ltl, lmk_sub, props_norepeat, seed, add_comma, model
            )
            if utt == utt_grounded:
                raise ValueError(f"ERROR\n{utt}\n==\n{utt_grounded}")
            if ltl == ltl_grounded:
                raise ValueError(f"ERROR\n{ltl}\n==\n{ltl_grounded}")
            utt_grounded.replace(
                ", , ", ", "
            )  # remove duplicated commas. artifact of adding commas in compose and lmk substitution
            data_grounded.append((utt_grounded, ltl_grounded))
            meta_data_grounded.append(
                (utt, ltl, pattern_type, props_norepeat, lmk_names, seed)
            )
...
def substitute_lmk(utt, ltl, lmks, props_norepeat, seed, add_comma, model):
    ...
    if (
        len(lmks) == nprops_norepeat
    ):  # already sample lmks from all perms for valid_iter
        lmk_names = lmks
    ...
    if add_comma:
        sub_map = {
            prop: f"{name}," for prop, name in zip(props_norepeat, lmk_names)
        }  # add comma after name for RER by GPT-3
    else:
        sub_map = {prop: name for prop, name in zip(props_norepeat, lmk_names)}
    utt_grounded = substitute_single_letter(utt, sub_map).strip(
        ","
    )  # if name at end of utt, remove extra comma if add_comma=True

    sub_map = {
        prop: name_to_prop(name, model) for prop, name in zip(props_norepeat, lmk_names)
    }
...
def construct_cleanup_res(env_res_dpath):
    ...
    sem_lmks = {
        re.sub(" +", " ", f"{size} {color} {shape} {amenity}".strip()): {}
        for color in colors
        for size in sizes
        for shape in shapes
    ...
    logging.info(
        f"generated {len(sem_lmks)} lmks for CleanUp\nfrom\nsizes: {sizes}\ncolors: {colors}\nshapes: {shapes}\namenities: {amenities}"
    )


def generate_prompts_from_grounded_split_dataset(
    split_fpath, prompt_dpath, nexamples, seed
):
    """
    ...
    for idx, ((utt, ltl), (_, _, pattern_type, props, _, _, _)) in enumerate(
        zip(train_iter, train_meta)
    ):
    ...
    prompt_fpath = (
        f"{prompt_dpath}/prompt_nexamples{nexamples}_{split_dataset_name}.txt"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--split_dpath",
        type=str,
        default="data/holdout_split_batch12_perm",
        help="dpath to all split datasets.",
    )
    parser.add_argument(
        "--domain",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="boston",
        choices=["boston", "all", "cleanup"],
        help="env lmks from 1 or all json files in data/osm/lmks or data/clenaup/lmks.",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="lang2ltl",
        choices=["lang2ltl", "copynet"],
        help="model name.",
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="random seed to sample lmks."
    )
    parser.add_argument(
        "--nsamples",
        type=int,
        default=2,
        help="number of samples per utt-ltl pair when perm lmks for valid set.",
    )
    parser.add_argument(
        "--remove_perm",
        action="store_false",
        help="True to keep prop perms in valid set. Default True.",
    )
    parser.add_argument(
        "--add_comma", action="store_true", help="True to add comma after lmk name."
    )
    parser.add_argument(
        "--env_prompt",
        type=str,
        default="boston",
        help="env dataset to generate full translation prompt.",
    )
    parser.add_argument(
        "--nexamples",
        action="store",
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="random seed to sample prompt examples.",
    )
    parser.add_argument(
        "--cleanup_re",
        action="store_true",
        help="True if construct referring expression json for CleanUp lmks.",
    )
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(model_dpath, f"log_gen_grounded_{args.env}.log"), mode="w"
            ),
    ...
    if args.env == "all":
        envs = [
            os.path.splitext(fname)[0][40:]
            for fname in os.listdir(domain_res_dpath)
    ...
    for env in envs:
        lmk2res = {
            entry[0]: entry[2:]
            for entry in load_from_file(
                os.path.join(
                    domain_res_dpath,
                    f"paraphrased-res-gpt4_filtered-attributes_{env}.csv",
                )
            )
        }
        ...
        if env == args.env_prompt:
            prompt_dpath = os.path.join(
                domain_dpath, "full_translation_prompt_diverse-re", args.env_prompt
            )
            os.makedirs(prompt_dpath, exist_ok=True)
            grounded_split_dpath = os.path.join(model_dpath, args.env_prompt)
            grounded_split_fpaths = [
                os.path.join(grounded_split_dpath, grounded_split_fname)
                for grounded_split_fname in os.listdir(grounded_split_dpath)
                if grounded_split_fname.endswith("pkl")
            ]
            for grounded_split_fpath in grounded_split_fpaths:
                for nexamples in args.nexamples:
                    generate_prompts_from_grounded_split_dataset(
                        grounded_split_fpath, prompt_dpath, nexamples, args.seed_prompt
                    )
