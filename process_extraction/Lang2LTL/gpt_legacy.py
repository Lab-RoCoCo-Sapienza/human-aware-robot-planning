...
class GPT3:
    ...
    def generate(self, query_prompt):
        ...
        while not complete:
            ...
            try:
                raw_responses = client.completions.create(
                    model=self.engine,
                    prompt=query_prompt,
                ...
                sleep(30)
                logging.info(
                    f"{ntries}: waiting for the server. sleep for 30 sec...\n{query_prompt}"
                )
    ...
    def get_embedding(self, text):  # engines must match when compare two embeddings
        ...
        embedding = client.embeddings.create(
            input=[text], model=self.engine
        )  # change for different embedding dimension)["data"][0]["embedding"]
        return embedding
...
def prompt2msg(query_prompt):
    ...
    for example in examples:
        if "\n" in example:
            ...
            q = "\n".join(
                example_splits[0:-1]
            )  # every line except the last in 1 example block
