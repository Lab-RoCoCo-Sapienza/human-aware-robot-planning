...
from utils import load_from_file, name_to_prop, save_to_file
...
def filter_cleanup(env):
    ...
    data, meta = [], []
    filter_dataset = [
        ["Pattern Type", "Number of Propositions", "Utterance", "True LTL"]
    ]
    ...
    print(
        f"Filter CleanUp dataset from Gopalan et al.: {len(dataset)} -> {len(filter_dataset)}"
    )
...
def filter_osm(env):
    ...
    print(
        f"Filter OSM dataset from Berg et al.: {len(dataset)} -> {len(filter_dataset)}"
    )
