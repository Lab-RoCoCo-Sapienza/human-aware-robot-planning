...
def compose(
    ...
    """
    # Load base dataset
    ltl2utts, all_base_ltls, all_base_ltls_spot, ltl2meta = load_base_dataset(
        data_fpath, logger
    )

    # Construct composed dataset
    composed_zeroshot = defaultdict(
        list
    )  # by itself serve as test set for zero-shot transfer
    formula2dataset = defaultdict(list)  # formula to (data, meta) pair
    composed_utt = [
        [] for _ in range(len(seeds_utt))
    ]  # utt split dataset for each fold
    nattempts, npairs = 0, 0
    err2count = defaultdict(
        int
    )  # composed formula: syntax error, infeasible, repeated, correct

    operator_seqs = list(
        product(compose_operators, repeat=nclauses - 1)
    )  # all combs of operators to connect base formulas
    base_ltl_seqs = list(
        product(all_base_ltls, repeat=nclauses)
    )  # all combs of base formulas

    for operator_seq in operator_seqs:
        for base_ltl_seq in base_ltl_seqs:
            base_ltl_seq = list(base_ltl_seq)
            logger.info(
                f"Composing {nattempts}:\n{operator_seq}\n{base_ltl_seq}\n{[ltl2meta[ltl] for ltl in base_ltl_seq]}"
            )
            data, meta = compose_single(
                operator_seq,
                base_ltl_seq,
                ltl2utts,
                all_base_ltls_spot,
            ...
            if data and meta:
                formula2dataset[meta[0][0]].append(
                    (data, meta)
                )  # key: composed ltl formula 'and-global_avoidance_3-visit_2'

                for seed_idx, seed_utt in enumerate(seeds_utt):
                    train_dataset, valid_dataset = train_test_split(
                        list(zip(data, meta)), test_size=size_utt, random_state=seed_utt
                    )
                    ...
                    if composed_utt[seed_idx]:
                        for item_idx, item in enumerate(
                            [train_data, train_meta, valid_data, valid_meta]
                        ):
                            composed_utt[seed_idx][item_idx] += item
                    else:
                        composed_utt[seed_idx] = [
                            train_data,
                            train_meta,
                            valid_data,
                            valid_meta,
                            {"size": size_utt, "seed": seed_utt},
                        ]
    ...
    logger.info(
        f"Composed types with syntax error: {err2count['syntax_err']}/{nattempts} = {err2count['syntax_err']/nattempts}"
    )
    logger.info(
        f"Composed types that are infeasible: {err2count['infeasible']}/{nattempts} = {err2count['infeasible']/nattempts}"
    )
    logger.info(
        f"Composed types being redundant: {err2count['repeat']}/{nattempts} = {err2count['repeat']/nattempts}"
    )
    ncorrects = (
        nattempts
        - err2count["syntax_err"]
        - err2count["infeasible"]
        - err2count["repeat"]
    )
    logger.info(
        f"Correct composed types: {ncorrects}/{nattempts} = {ncorrects /nattempts}"
    )
    logger.info(f"Total number of composed pairs: {npairs}")

    # Save zero_shot transfer dataset
    save_fpath_zeoshot = os.path.join(
        os.path.dirname(data_fpath), f"composed_zeroshot_{Path(data_fpath).stem}.pkl"
    )
    save_to_file(composed_zeroshot, save_fpath_zeoshot)

    # Save utterance holdout dataset
    save_fpath_utt = os.path.join(
        os.path.dirname(data_fpath), f"composed_utt_{Path(data_fpath).stem}.pkl"
    )
    ...
    kf = KFold(
        n_splits=len(all_formulas) // size_formula,
        random_state=seed_formula,
        shuffle=True,
    )
    for fold_idx, (train_indices, holdout_indices) in enumerate(kf.split(all_formulas)):
        ...
        for formula in all_formulas:
            for data, meta in formula2dataset[
                formula
        ...
        composed_formula.append(
            (
                train_iter,
                train_meta,
                valid_iter,
                valid_meta,
                {"size": size_formula, "seed": seed_formula, "fold_idx": fold_idx},
            )
        )
    save_fpath_formula = os.path.join(
        os.path.dirname(data_fpath), f"composed_formula_{Path(data_fpath).stem}.pkl"
    )
...
def compose_single(
    ...
    all_base_triples = [
        [(utt, base_ltl, ltl2meta[base_ltl]) for utt in ltl2utts[base_ltl]]
        for base_ltl in base_ltls
    ]

    # Compose one operator at a time
    for operator in operators:
        if operator in UNARY_OPERATORS:
            base_triples = all_base_triples.pop(
                0
            )  # replace clause 1 by composed formula
        elif operator in BINARY_OPERATORS:
            if len(all_base_triples) < 2:
                raise ValueError(
                    f"ERROR: {len(all_base_triples)} bases not enough for binary operator {operator}."
                )
            base_triples = [
                all_base_triples.pop(0),
                all_base_triples.pop(0),
            ]  # replace clause 1 and 2 by composed formula
        else:
            raise ValueError(f"ERROR: unrecognized operator: {operator}.")

        if len(base_triples) > 1:  # binary operator
            base_triples_comb = list(
                product(*base_triples)
            )  # all combs of operand 1 and operand 2
        else:  # unary operator
            base_triples_comb = base_triples
        logger.info(
            f"Number of triples to be composed: {len(base_triples_comb)}\n"
        )  # TODO: count only valid <= 2 clauses

        pairs_composed = []
        for pair_idx, composing_bases in enumerate(base_triples_comb):
            utts_base, ltls_base, metas = zip(
                *composing_bases
            )  # unpack list of tuples into 3 lists
            if operator == "and":
        ...
        all_base_triples.insert(
            0, pairs_composed
        )  # continue composing composed formula with remaining base formulas
    ...
    for utt, ltl, metas in dataset_composed:
        ...
        info_composed = "-".join(
            list(operators)
            + [f"{pattern_type}_{len(props)}" for pattern_type, props in metas]
        )
        info_bases = "-".join(
            list(operators)
            + [f"{pattern_type}_{'_'.join(props)}" for pattern_type, props in metas]
        )
        meta.append(
            (info_composed, info_bases)
        )  # two differ only for perm dataset ('and-global_avoidance_3-visit_2', 'and-patrolling_d_a_b-visit_a_b')
    return data, meta
...
def get_valid_composed_formulas(data_fpath, nclauses, compose_operators):
    ...
    log_fpath = os.path.join(
        os.path.dirname(data_fpath),
        f"composed_valid_formulas_stats_{Path(data_fpath).stem}.log",
    )
    logging.basicConfig(
        level=logging.DEBUG,
        format="%(message)s",
    ...
    err2count = defaultdict(
        int
    )  # composed formula: correct, syntax error, infeasible, repeat
    nattempts = 0
    composed_ltls = set()
    for operator_seq in operator_seqs:
        for base_ltl_seq in base_ltl_seqs:
            ...
            for operator in operator_seq:
                if operator in UNARY_OPERATORS:
                    ...
                    if len(base_ltl_seq) < 2:
                        raise ValueError(
                            f"ERROR: {len(base_ltl_seq)} not enough base ltls for binary operator {operator}."
                        )
                    base_ltls = [
                        base_ltl_seq.pop(0),
                        base_ltl_seq.pop(0),
                ...
                if ltl_spot in all_base_ltls_spot:
                    err2count["repeat"] += 1
                    logger.info(
                        f"Repeated composed formula already exists in base dataset:\n{ltl_spot} = {ltl_composed}\n{base_ltls}\n"
                    )
                    break
                elif spot.are_equivalent(ltl_spot, spot.formula("False")):
                    err2count["infeasible"] += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_fpath",
        type=str,
        default="data/symbolic_batch12_noperm.csv",
        help="base dataset to compose.",
    )
    parser.add_argument(
        "--nclauses", type=int, default=2, help="number of clauses in composed formula."
    )
    parser.add_argument(
        "--get_formula_stats",
        action="store_true",
        help="True to only compute stats on composed formulas.",
    )
    parser.add_argument(
        "--ignore_repeat",
        action="store_true",
        help="True to ignore composed formulas already exist.",
    )
    parser.add_argument(
        "--size_formula", type=int, default=900, help="fold size for formula holdout."
    )
    parser.add_argument(
        "--seed_formula", type=int, default=42, help="seed for formula holdout."
    )
    parser.add_argument(
        "--size_utt", type=float, default=0.2, help="fold size for utterance holdout."
    )
    parser.add_argument(
        "--seeds_utt",
        type=int,
        default=[0, 1, 2, 42, 111],
        help="seed for utterance holdout.",
    )
    args = parser.parse_args()

    log_fpath = os.path.join(
        os.path.dirname(args.data_fpath), f"composed_{Path(args.data_fpath).stem}.log"
    )
    logging.basicConfig(
        level=logging.DEBUG,
        format="%(message)s",
