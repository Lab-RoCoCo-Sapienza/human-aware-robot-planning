...
def plot_symbolic_accuracies2():
    ...
    with sns.plotting_context("poster", rc=rc):
        plt.figure(figsize=[12, 10])
        sns.barplot(
            data=df, x="Test Type", y="Accuracy", hue="Model", alpha=0.85, capsize=0.1
        )
        plt.ylim([0, 1])
    ...
    for test_type in SYMBOLIC_TEST_TYPES:
        for model_type in SYMBOLIC_MODEL_TYPES:
            ...
            with sns.plotting_context(context="poster", rc=rc):
                ...
                plt.figure(figsize=[12, 10])
                sns.barplot(
                    data=accs, x="Accuracy", y="Formula Type", color="b", ci=None
                )
                plt.xlim([0, 1])
                plt.savefig(
                    f"figures/type_accs_{test_type}_{model_type}.jpg",
                    dpi=400,
                    bbox_inches="tight",
                )
    ...
    for test_type in SYMBOLIC_TEST_TYPES:
        for model_type in SYMBOLIC_MODEL_TYPES:
            ...
            with sns.plotting_context(context="poster", rc=rc):
                ...
                plt.figure(figsize=[12, 10])
                sns.barplot(
                    data=accs, x="N Propositions", y="Accuracy", color="b", capsize=0.1
                )
                plt.ylim([0, 1])
                plt.savefig(
                    f"figures/n_prop_accs_{test_type}_{model_type}.jpg",
                    dpi=400,
                    bbox_inches="tight",
                )
...
def plot_per_city_accuracy():
    for test_type in SYMBOLIC_TEST_TYPES:
        ...
        with sns.plotting_context(context="poster", rc=rc):
            ...
            plt.xlim([0, 1])
            plt.savefig(
                f"figures/city_accuracies_{test_type}.jpg", dpi=400, bbox_inches="tight"
            )
...
def symbolic_error_piechart_finetuned_formula():
    ...
    with sns.plotting_context(context="poster", rc=rc):
        ...
        plt.pie(data, labels=labels, colors=colors)
        plt.savefig(
            "figures/symbolic_piechart_finetuned_formula.jpg",
            dpi=400,
            bbox_inches="tight",
        )
...
def symbolic_error_piechart_finetuned_type():
    ...
    with sns.plotting_context(context="poster", rc=rc):
        ...
        plt.pie(data, labels=labels, colors=colors)
        plt.savefig(
            "figures/symbolic_piechart_finetuned_type.jpg", dpi=400, bbox_inches="tight"
        )
...
def plot_full_system_piechart(test_type):
    ...
    with sns.plotting_context(context="poster", rc=rc):
        ...
        plt.savefig(
            f"figures/full_system_{test_type}.jpg", dpi=400, bbox_inches="tight"
        )
