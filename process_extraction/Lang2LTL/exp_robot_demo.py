...
if __name__ == "__main__":
    ...
    result_fpath = os.path.join(
        result_dpath,
        f"log_robot-demo_{datetime.now().strftime('%m-%d-%Y-%H-%M-%S')}.log",
    )
    logging.basicConfig(
        level=logging.INFO,
        format="%(message)s",
        handlers=[logging.FileHandler(result_fpath, mode="w"), logging.StreamHandler()],
    )
    ...
    env_fpath = (
        ...
        else os.path.join(SHARED_DPATH, "data/robot_demo_configs/robot_demo_envs.yaml")
    )
