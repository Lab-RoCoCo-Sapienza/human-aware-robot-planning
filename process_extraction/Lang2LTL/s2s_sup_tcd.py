...
class Seq2Seq:
    def __init__(self, model_dpath, model_name, **kwargs):
        ...
        if "t5" in model_name or "bart" in model_name:
            ...
            self.model = AutoModelForSeq2SeqLM.from_pretrained(model_dpath).to(
                self.device
            )
    ...
    def type_constrained_decode(self, utts):
        ...
        def mask_and_regen(lm_logits, next_decoder_input_ids):
            ...
            next_token = self.tokenizer.decode(
                next_decoder_input_ids[0], skip_special_tokens=False
            )
            return next_token, next_decoder_input_ids, lm_logits

        def add_and_gen_new(
            lm_logits, next_decoder_input_ids, decoder_input_ids, next_token, whitespace
        ):
            ...
            while "</s>" not in token_list and len(token_list) < MAX_LENGTH:
                ...
                if len(token_list) == 0:
                    if next_token in PROPS + UNARY_OPERATORS + BINARY_OPERATORS:
                        ...
                        next_token, next_decoder_input_ids, lm_logits = mask_and_regen(
                            lm_logits, next_decoder_input_ids
                        )
                # after the first token
                # only check depth after certain # of operators & props
                elif len(token_list) > CHECK_DEPTH:
                    no_uni_list = "".join(
                        [s for s in token_list if not s in UNARY_OPERATORS]
                    )
                    ...
                    if depth(partial_tree) > MAX_DEPTH:
                        ...
                        if not (
                            ...
                            (
                                ...
                                next_decoder_input_ids,
                                lm_logits,
                            ) = mask_and_regen(lm_logits, next_decoder_input_ids)
                ...
                elif next_token in PROPS + UNARY_OPERATORS + BINARY_OPERATORS + [
                    END_TOKEN
                ]:
                    if not is_valid(token_list, next_token):
                        next_token, next_decoder_input_ids, lm_logits = mask_and_regen(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="checkpoint to use for inferance."
    )
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                f"results/s2s_tcd_{args.model}_{Path(args.data_fpath).stem}_{Path(args.model_dpath).stem}.log",
                mode="w",
            ),
    ...
    for data_fpath in data_fpaths:
        ...
        evaluate_sym_trans(
            s2s, data_fpath, result_log_fpath, analysis_fpath, acc_fpath, batch_size=30
        )
