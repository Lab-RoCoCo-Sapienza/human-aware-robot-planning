...
from utils import load_from_file, name_to_prop, save_to_file, substitute_single_word
...
def evaluate_lang(
    true_ltls, out_ltls, true_names, out_names, out_grnds, convert_rule, all_props
):
    accs = []
    for true_ltl, out_ltl, true_name, out_name, out_grnd in zip(
        true_ltls, out_ltls, true_names, out_names, out_grnds
    ):
        if (
            out_ltl == true_ltl
        ):  # Spot cannot handle long but correct LTL formula, e.g. F & 62_on_the_park U 62_on_the_park & ! 62_on_the_park U ! 62_on_the_park F & 62_on_the_park U 62_on_the_park & ! 62_on_the_park U ! 62_on_the_park F & 62_on_the_park U 62_on_the_park & ! 62_on_the_park U ! 62_on_the_park F 62_on_the_park
            is_correct = "True"
        else:
            try:  # output LTL formula may have syntax error
                spot_correct = spot.are_equivalent(
                    spot.formula(true_ltl), spot.formula(out_ltl)
                )
                is_correct = "True" if spot_correct else "False"
            except SyntaxError:
                logging.info(
                    f"Syntax error OR formula too long:\n{true_ltl}\n{out_ltl}"
                )
                # breakpoint()

                if set(true_name) == set(out_grnd):
                    true_props = [
                        name_to_prop(name, convert_rule) for name in true_name
                    ]
                    true_sub_map = {
                        prop: sym
                        for prop, sym in zip(true_props, all_props[: len(true_props)])
                    }
                    ...
                    out_sub_map = {
                        prop: sym
                        for prop, sym in zip(out_props, all_props[: len(out_props)])
                    }
                    ...
                    try:  # output LTL formula may have syntax error
                        spot_correct = spot.are_equivalent(
                            spot.formula(true_ltl_short), spot.formula(out_ltl_short)
                        )
                        is_correct = "True" if spot_correct else "False"
                    except SyntaxError:
                        logging.info(
...
def evaluate_lang_new(
    true_ltls, out_ltls, true_sym_ltls, out_sym_ltls, true_names, out_names, out_grnds
):
    accs = []
    for (
        true_ltl,
        ...
        if true_ltl == out_ltl:
            ...
            try:  # output LTL formula may have syntax error
                spot_correct = spot.are_equivalent(
                    spot.formula(true_sym_ltl), spot.formula(out_sym_ltl)
                )
                if spot_correct:
                    if set(true_name) == set(
                        out_name
                    ):  # TODO: check only work if RE == lmk_name when generate grounded dataset
                        if set(true_name) == set(out_grnd):
                            is_correct = "True"
                        else:
...
def evaluate_lang_single(
    ...
    for batch in batch(list(zip(valid_iter, valid_meta)), 1):  # batch_size = 100
        ...
        for idx, ((utt, true_ltl), (pattern_type, prop_perm, *other_meta)) in enumerate(
            batch
        ):
            nsamples += 1
            nprops = len(prop_perm)
            ...
            try:  # output LTL formula may have syntax errorx
                ...
                is_correct = spot.are_equivalent(
                    spot.formula(out_ltl), spot.formula(true_ltl)
                )
                is_correct = "True" if is_correct else "False"
            except:
                is_correct = "Syntax Error"
            ...
            if is_correct == "True":
                ncorrects += 1
            logging.info(
                f"partial results: {ncorrects}/{nsamples} = {ncorrects/nsamples}\n"
            )
    save_to_file(result_log, result_log_fpath)

    meta2acc = {
        meta: np.mean([True if acc == "True" else False for acc in accs])
        for meta, accs in meta2accs.items()
    }
    ...
    total_acc = np.mean(
        [
            True if acc == "True" else False
            for accs in meta2accs.values()
            for acc in accs
        ]
    )
...
def evaluate_lang_from_file(
    model, split_dataset_fpath, analysis_fpath, result_log_fpath, acc_fpath
):
    _, _, valid_iter, valid_meta = load_split_dataset(split_dataset_fpath)
    # valid_iter = valid_iter[:10]
    # valid_meta = valid_meta[:10]
    return evaluate_lang_single(
        model,
        valid_iter,
        valid_meta,
        analysis_fpath,
...
def aggregate_results(result_fpaths, filter_types):
    ...
    result_fnames = [
        os.path.splitext(result_fpath)[0] for result_fpath in result_fpaths
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_dataset_fpath",
        type=str,
        default="data/holdout_split_batch12_perm/symbolic_batch12_perm_utt_0.2_111.pkl",
        help="path to pkl file storing train set",
    )
    parser.add_argument(
        "--test_dataset_fpath",
        type=str,
        default="data/holdout_split_batch12_perm/symbolic_batch12_perm_utt_0.2_111.pkl",
        help="path to pkl file storing test set",
    )
    parser.add_argument(
        "--analysis_fpath",
        type=str,
        default="data/analysis_symbolic_batch12_perm.csv",
        help="path to dataset analysis",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="gpt3_finetuned_symbolic_batch12_perm_utt_0.2_111",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of examples per instance for GPT-3",
    )
    parser.add_argument(
        "--aggregate",
        action="store_true",
        help="whether to aggregate results or compute new results.",
    )
    ...
    if args.aggregate:  # aggregate acc-per-formula result files
        ...
        result_fpaths = [
            os.path.join(result_dpath, fname)
            for fname in os.listdir(result_dpath)
            if "acc" in fname and "csv" in fname and "aggregated" not in fname
        ]
        filter_types = ["fair_visit"]
        accumulated_acc, accumulated_std = aggregate_results(
            result_fpaths, filter_types
        )
        print("Please verify results files")
        pprint(result_fpaths)
    else:
        if (
            "gpt3" in args.model or "davinci" in args.model
        ):  # gpt3 for finetuned gpt3, davinci for off-the-shelf gpt3
            ...
            if (
                "utt" in args.train_dataset_fpath
            ):  # results directory based on holdout type
            ...
            if "finetuned" in args.model:
                ...
                result_log_fpath = os.path.join(
                    result_dpath, f"log_{args.model}.csv"
                )  # fintuned model name already contains dataset name
                acc_fpath = os.path.join(result_dpath, f"acc_{args.model}.csv")
            else:
                engine = args.model
                prompt_fpath = os.path.join(
                    "data",
                    "prompt_nl2ltl",
                    f"nl2ltl_prompt_nexamples{args.nexamples}_{dataset_name}.txt",
                )
                ...
                os.makedirs(result_dpath, exist_ok=True)
                result_log_fpath = os.path.join(
                    result_dpath, f"log_{args.model}_{dataset_name}.csv"
                )
                acc_fpath = os.path.join(
                    result_dpath, f"acc_{args.model}_{dataset_name}.csv"
                )
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    f"{os.path.splitext(result_log_fpath)[0]}.log", mode="w"
                ),
                logging.StreamHandler(),
            ],
        )

        evaluate_lang_from_file(
            model, split_dataset_fpath, args.analysis_fpath, result_log_fpath, acc_fpath
        )
