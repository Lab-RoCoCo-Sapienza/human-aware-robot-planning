...
from .utils import build_placeholder_map, load_from_file, save_to_file, substitute


SHARED_DPATH = os.path.join(
    os.path.expanduser("~"), "data", "shared", "lang2ltl"
)  # group's data folder on cluster
...
def lang2ltl(
    ...
    rer_model="gpt4",
    rer_engine="gpt-4",
    rer_prompt_fpath=f"{SHARED_DPATH}/data/rer_prompt_diverse_16.txt",
    embed_model="gpt3",
    embed_engine="text-embedding-ada-002",
    ground_model="gpt3",
    topk=2,
    update_embed=True,
    model_dpath=f"{SHARED_DPATH}/model_3000000",
    ...
    if sym_trans_model in HF_MODELS:
        ...
        translation_engine = load_from_file(
            os.path.join(model_dpath, "gpt3_models.pkl")
        )[translation_engine]
    else:
        raise ValueError(
    ...
    sym_utts, sym_ltls, out_ltls, placeholder_maps = translate_grounded_utts(
        ground_utts,
        objs_per_utt,
        sym_trans_model,
        translation_engine,
...
def rer(rer_model, rer_engine, rer_prompt, input_utts):
    ...
    for idx_utt, utt in enumerate(input_utts):
        ...
        names_per_utt = [
            name.strip()
            for name in rer_module.extract_re(
...
def ground_res(res, re2embed_fpath, obj_embed, ground_model, embed_engine, topk):
    ...
    if os.path.exists(
        re2embed_fpath
    ):  # load cached embeddings of referring expressions
...
def translate_grounded_utts(
    ...
    for objs in objs_per_utt:
        placeholder_map, placeholder_map_inv = build_placeholder_map(
            objs, convert_rule, props
        )
        placeholder_maps.append(placeholder_map)
        placeholder_maps_inv.append(placeholder_map_inv)
    symbolic_utts, _ = substitute(
        ground_utts, placeholder_maps, is_utt=True
