...
class Seq2SeqTransformer(nn.Module):
    ...
    def encode(self, src, src_mask):
        return self.transformer.encoder(
            self.positional_encoding(self.src_token_embed(src)), src_mask
        )

    def decode(self, tar, memory, tar_mask):
        return self.transformer.decoder(
            self.positional_encoding(self.tar_token_embed(tar)), memory, tar_mask
        )
...
def construct_dataset_meta(train_iter):
    ...
    for ln in [SRC_LANG, TAR_LANG]:
        vocab_transform[ln] = build_vocab_from_iterator(
            yield_tokens(train_iter, tokenizer, ln),
            min_freq=1,
        ...
        vocab_transform[ln].set_default_index(
            UNK_IDX
        )  # default index returned when token not found
    src_vocab_size, tar_vocab_size = len(vocab_transform[SRC_LANG]), len(
        vocab_transform[TAR_LANG]
    )

    text_transform = {
        SRC_LANG: sequential_transforms(
            tokenizer, vocab_transform[SRC_LANG], tensor_transform
        ),
        TAR_LANG: sequential_transforms(
            tokenizer, vocab_transform[TAR_LANG], tensor_transform
        ),
...
def train_epoch(model, optimizer, train_iter):
    ...
    for src_batch, tar_batch in train_dataloader:
        ...
        src_mask, tar_mask, src_padding_mask, tar_padding_mask = create_mask(
            src_batch, tar_input
        )
...
def evaluate(model, valid_iter):
    ...
    valid_dataloader = DataLoader(
        valid_iter, batch_size=BATCH_SIZE, collate_fn=collate_fn
    )
...
def translate(model, vocab_transform, text_transform, src_sentence):
    ...
    tar_tokens = greedy_decode(
        model, src, src_mask, max_len=num_tokens + 5, start_symbol=SOS_IDX
    ).flatten()
    return (
        " ".join(
            vocab_transform[TAR_LANG].lookup_tokens(list(tar_tokens.cpu().numpy()))
        )
        .replace("<sos>", "")
        .replace("<eos>", "")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--split_dataset_fpath",
        type=str,
        default="data/split_symbolic_no_perm_batch1_ltl_type_2_42.pkl",
        help="complete file path or prefix of file paths to train and test data for supervised seq2seq",
    )
    args = parser.parse_args()

    if (
        "pkl" in args.split_dataset_fpath
    ):  # complete file path, e.g. data/split_symbolic_no_perm_batch1_utt_0.2_42.pkl
        split_dataset_fpaths = [args.split_dataset_fpath]
    else:  # prefix of file paths, e.g. split_symbolic_no_perm_batch1_utt
        split_dataset_fpaths = [
            os.path.join("data", fpath)
            for fpath in os.listdir("data")
            if args.split_dataset_fpath in fpath
        ]

    for split_dataset_fpath in split_dataset_fpaths:
        # Load train, test data
        train_iter, train_meta, valid_iter, valid_meta = load_split_dataset(
            split_dataset_fpath
        )
        (
            vocab_transform,
        ...
        writer = (
            SummaryWriter()
        )  # writer will output to ./runs/ directory by default; activate: tensorboard --logdir=runs
        ...
        for epoch in range(1, NUM_EPOCHS + 1):
            ...
            print(
                f"Epoch: {epoch}, Train loss: {train_loss:.3f}, Val loss: {valid_loss:.3f}\n"
