...
def plot_cm(results_fpath, cm, all_types):
    ...
    if "utt" in results_fpath:
        ...
        raise ValueError(
            f"ERROR: unrecognized holdout type in results file:\n{results_fpath}"
        )
    ...
    sns.heatmap(
        ...
        left=0.2, bottom=0.25
    )  # change vmax for different saturation
    fig_fpath = os.path.join(
        os.path.dirname(results_fpath),
        f"fig_{'_'.join(Path(results_fpath).stem.split('_')[1:])}.jpg",
    )  # remove results ID: "log"
    plt.setp(
...
def analyze_errs(result_fpath, type2nprops, debug):
    ...
    formula2type, formula2prop = find_all_formulas(
        type2nprops, "noperm" in result_fpath
    )
    if debug:
        print(
            f"Total number of unique LTL formulas: {len(formula2type)}, {len(formula2prop)}"
        )
    ...
    for idx, result in enumerate(results):
        ...
        result = result[
            1:
        ]  # remove train_or_valid column because all results are valid
        (
            pattern_type,
            nprops,
            true_prop_perm_str,
        ...
        if is_correct == "Syntax Error":
            ...
            if output_ltl in formula2type:
                ...
                if formula2type[true_ltl] != formula2type[output_ltl]:
                    ...
                    true_prop_perm != pred_prop_perm and is_correct == "False"
                ):  # diff prop order and not spot equivalent, e.g visit 2
    ...
    formula2nutts_sorted = sorted(
        formula2nutts.items(), key=lambda kv: kv[1], reverse=True
    )
    print(formula2nutts_sorted)
    out_str += f"{formula2nutts_sorted}\n"

    type2errs_sorted = sorted(
        type2errs.items(), key=lambda kv: len(kv[1]), reverse=True
    )
    nerrs_caught = 0
    for typ, errs in type2errs_sorted:
        print(f"number of {typ}:\t{len(errs)}/{total_errs}\t= {len(errs)/total_errs}")
        out_str += (
            f"number of {typ}:\t{len(errs)}/{total_errs}\t= {len(errs)/total_errs}\n"
        )
        nerrs_caught += len(errs)
    print(
        f"number of all errors:\t{total_errs}/{len(results)}\t= {total_errs / len(results)}\n"
    )
    ...
    if total_errs != nerrs_caught:
        raise ValueError(
            f"total nerrors != nerrs_caught: {total_errs} != {nerrs_caught}"
        )

    all_types = sorted(type2nprops.keys(), reverse=True)
    cm = confusion_matrix(
        y_true, y_pred, labels=all_types, normalize="true"
    )  # y/rows: true; x/cols: predicted
...
def acc_osm_cities(
    env_dpath="results/lang2ltl/osm/",
    filtered=["boston_e2e"],
    holdouts=["utt", "formula"],
):
    def acc_one_city(df):
    ...
    holdout2dame = {
        "utt": "utt_holdout_batch12",
        "formula": "formula_holdout_batch12",
        "type": "type_holdout_batch12",
    }
    ...
    for city in cities:
        if city not in filtered:
            ...
            for holdout_dname in holdout_dnames:
                ...
                result_fnames = [
                    fname
                    for fname in os.listdir(holdout_dpath)
                    if fname.endswith("csv")
                ]
                city2holdout2acc[city][holdout_dname] = [
                    acc_one_city(pd.read_csv(os.path.join(holdout_dpath, result_fname)))
    ...
    for city in cities:
        if city not in filtered:
            per_city = {}
            for holdout_dname in holdout_dnames:
                per_city[holdout_dname] = {
                    "mean": np.mean(city2holdout2acc[city][holdout_dname]),
                    "std": np.std(city2holdout2acc[city][holdout_dname]),
                }
            final_acc["analyzed"][city] = per_city

    save_to_file(
        final_acc,
        os.path.join(env_dpath, f"aggregated_acc_{'&'.join(holdouts)}_per_city.json"),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--result_path",
        type=str,
        default="results/finetuned_gpt3/formula_holdout_batch12_perm",
        help="fpath or dpath of holdout result to analyze.",
    )
    parser.add_argument(
        "--split_fpath",
        type=str,
        default="data/holdout_split_batch12_perm/symbolic_batch12_perm_utt_0.2_42.pkl",
        help="fpath to split dataset used to produce results.",
    )
    parser.add_argument(
    ...
    result_fpaths = (
        [
            os.path.join(args.result_path, fname)
            for fname in os.listdir(args.result_path)
            if "log" in fname and "csv" in fname
        ]
        if os.path.isdir(args.result_path)
        else [args.result_path]
    )
    analysis_fpath = (
        os.path.join(args.result_path, f"analysis_{Path(args.result_path).stem}.txt")
        if os.path.isdir(args.result_path)
        else os.path.join(
            os.path.dirname(args.result_path),
            f"analysis_{Path(args.result_path).stem}.txt",
        )
    )
    all_type2errs, all_nresults, all_out_str = defaultdict(list), 0, ""
    for result_fpath in result_fpaths:
        cm, all_types, nresults, type2errs_sorted, out_str = analyze_errs(
            result_fpath, TYPE2NPROPS, args.debug
        )
    ...
    merged_csv_fpath = os.path.join(
        args.result_path, f"merged_{Path(args.result_path).stem}.csv"
    )
    pd.concat(df_list, axis=0).iloc[:, 1:].to_csv(merged_csv_fpath)
    cm, all_types, nresults, type2errs_sorted, out_str = analyze_errs(
        merged_csv_fpath, TYPE2NPROPS, args.debug
    )
    plot_cm(merged_csv_fpath, cm, all_types)

    type2errs_sorted = sorted(
        all_type2errs.items(), key=lambda kv: len(kv[1]), reverse=True
    )
    nerrs_caught = 0
    for typ, errs in type2errs_sorted:
        nerrs_caught += len(errs)
    for typ, errs in type2errs_sorted:
        print(
            f"number of {typ}:\t{len(errs)}/{nerrs_caught}\t= {len(errs) / nerrs_caught}"
        )
        all_out_str += f"number of {typ}:\t{len(errs)}/{nerrs_caught}\t= {len(errs) / nerrs_caught}\n"
    print(
        f"total number of errors:\t{nerrs_caught}/{all_nresults}\t= {nerrs_caught / all_nresults}"
    )
