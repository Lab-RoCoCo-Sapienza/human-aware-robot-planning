...
def substitute_single_word(in_str, sub_map):
    ...
    sub_map = sorted(
        sub_map.items(), key=lambda kv: len(kv[0]), reverse=True
    )  # start substitution with long strings
    subs_done = set()
...
def remove_prop_perms(data, meta, all_props):
    ...
    for (utt, ltl), (pattern_type, props) in zip(data, meta):
        props = list(props)
        if props.count(props[0]) == len(
            props
        ):  # restricted avoidance formulas all props are the same
            props_noperm = [all_props[0]] * len(props)  # b, b -> a, a
        else:
            props_noperm = all_props[: len(props)]
        sub_map = {
            old_prop: new_prop for old_prop, new_prop in zip(props, props_noperm)
        }
        utt_noperm = substitute_single_letter(utt, sub_map)
        ltl_noperm = substitute_single_letter(ltl, sub_map)
        formula2data[(pattern_type, tuple(props_noperm))].append(
            (utt_noperm, ltl_noperm)
        )
    ...
    for (pattern_type, props_noperm), data in formula2data.items():
        data = list(
            dict.fromkeys(data)
        )  # remove duplicate utt structures per formula. same order across runs
        for utt, ltl in data:
...
def sample_small_dataset(data_fpath):
    ...
    for (utt_grounded, ltl_grounded), (
        ...
        props = list(props)
        formula2data[(pattern_type, len(props))].append(
            (
                utt_grounded,
                ltl_grounded,
                utt,
    ...
    save_fpath = os.path.join(
        os.path.dirname(data_fpath), f"small_{os.path.basename(data_fpath)}"
    )
...
def name_to_prop(name, convert_rule):
    ...
    if convert_rule == "lang2ltl":
        return "_".join(
            name.translate(str.maketrans("/()-–", "     ", "'’,.!?")).lower().split()
        )
...
def deserialize_props_str(props_str):
    ...
    props = [
        ...
        for prop in list(props_str.strip("()[]").split(", "))
    ]
    return props
...
def append_ids_to_path(pth, appends, id_trues, id_falses):
    ...
    for append, id_true, id_false in zip(appends, id_trues, id_falses):
        if os.path.isfile(pth):
            ...
            pth = (
                f"{base_pth}_{id_true}{os.path.splitext(pth)[1]}"
                if append
                else f"{base_pth}_{id_false}{os.path.splitext(pth)[1]}"
            )
        else:
...
def remove_id_from_path(pth, identifier):
    fname = [
        fname_sub for fname_sub in Path(pth).stem.split("_") if fname_sub != identifier
    ]
    return os.path.join(
        os.path.dirname(pth), f"{'_'.join(fname)}{os.path.splitext(pth)[1]}"
    )
...
def count_lmk_ntokens(model):
    ...
    cities = [
        ...
        for fname in os.listdir(env_lmks_dpath)
        if "json" in fname
    ]
    for city in cities:
