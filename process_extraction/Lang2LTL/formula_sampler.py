...
ALL_TYPES = [
    ...
    "sequenced_visit",
    "ordered_visit",
    "strictly_ordered_visit",
    "fair_visit",
    "patrolling",
    "sequenced_patrolling",
    "ordered_patrolling",
    "strictly_ordered_patrolling",
    "fair_patrolling",  # batch1
    "past_avoidance",
    "global_avoidance",
    "future_avoidance",
...
FILTER_TYPES = [
    ...
    "fair_patrolling",  # batch1
    "instantaneous_reaction",
    "bound_reaction",  # batch2
]
FEASIBLE_TYPES = [typ for typ in ALL_TYPES if typ not in FILTER_TYPES]
TYPE2NPROPS = {
    "visit": [1, 2, 3, 4, 5],
    "sequenced_visit": [2, 3, 4, 5],
    "ordered_visit": [2, 3, 4, 5],
    "strictly_ordered_visit": [2, 3, 4, 5],
    "patrolling": [1, 2, 3, 4, 5],  # batch1
    "past_avoidance": [2],
    "global_avoidance": [1, 2, 3, 4, 5],
    "future_avoidance": [2],
    "upper_restricted_avoidance": [1, 2, 3, 4, 5],
    "lower_restricted_avoidance": [1, 2, 3, 4, 5],
    "exact_restricted_avoidance": [1, 2, 3, 4, 5],
    "delayed_reaction": [2],
    "prompt_reaction": [2],
    "bound_delay": [2],
    "wait": [2],  # batch2
...
def strict_ordered_visit_constraint3(props):
    assert (
        len(props) > 1
    ), f"length of props for strict_ordered_visit_constraint3 must be > 1, got {len(props)}"
    if len(props) == 2:
        a, b = props[0], props[1]
...
def fair_visit_fixed(props):  # TODO: to be fixed
    ...
    if len(props) > 1:
        props.append(
            props[0]
        )  # proposition list circles back to 1st proposition for 2nd constraint
...
def fair_visit_constraint2(props):
    ...
    assert (
        len(props) > 1
    ), f"length of props for fair_visit_constraint2 must be > 1, got {len(props)}"
    if len(props) == 2:
        a, b = props[0], props[1]
...
def ordered_patrol_constraint3(props):
    ...
    assert (
        len(props) > 1
    ), f"length of props for ordered_patrol_constraint3 must be > 1, got {len(props)}"
    if len(props) == 2:
        a, b = props[0], props[1]
...
def strict_ordered_patrol_constraint4(props):
    ...
    assert (
        len(props) > 1
    ), f"length of props for strict_ordered_patrol_constraint4 must be > 1, got {len(props)}"
    if len(props) == 2:
        a, b = props[0], props[1]
        return f"G i {a} W {a} & ! {a} W ! {a} {b}"
    b, a = props[1], props.pop(0)
    return f"& G i {a} W {a} & ! {a} W ! {a} {b} " + strict_ordered_patrol_constraint4(
        props
    )
...
def past_avoid(props):
    assert (
        len(props) == 2
    ), f"length of props for past_avoid must be 2, got {len(props)}"
...
def future_avoid(props):
    assert (
        len(props) == 2
    ), f"length of props for future_avoid must be 2, got {len(props)}"
...
def instantaneous_reaction(props):
    assert (
        len(props) == 2
    ), f"length of props for instantaneous_reaction must be 2, got {len(props)}"
...
def delayed_reaction(props):
    assert (
        len(props) == 2
    ), f"length of props for delayed_reaction must be 2, got {len(props)}"
...
def prompt_reaction(props):
    assert (
        len(props) == 2
    ), f"length of props for prompt_reaction must be 2, got {len(props)}"
...
def bound_reaction(props):
    assert (
        len(props) == 2
    ), f"length of props for bound_reaction must be 2, got {len(props)}"
...
def bound_delay(props):
    assert (
        len(props) == 2
    ), f"length of props for bound_delay must be 2, got {len(props)}"
...
def utils(props):
    ...
    assert (
        len(props) > 1
    ), f"length of props for conjunction of utils must be > 1, got {len(props)}"
    if len(props) == 2:
        a, b = props[0], props[1]
        return f"U ! {b} {a}"
    b, a = props[1], props.pop(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pattern_type",
        type=str,
        default="exact_restricted_avoidance",
        help="type of specification pattern.",
    )
    parser.add_argument("--nprops", type=int, default=2, help="number of propositions.")
    parser.add_argument(
        "--debug",
        action="store_true",
        help="include to show LTL formulas in Spot instead of string.",
    )
